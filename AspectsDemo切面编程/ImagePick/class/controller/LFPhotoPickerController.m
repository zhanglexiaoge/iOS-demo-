        ...
        if (self.doneButtonClickBlock) {
            ...
            dispatch_globalQueue_async_safe((^{
                ...
                if (imagePickerVc.selectedModels.count) {
                    ...
                    void (^resultComplete)(LFResultObject *, NSInteger,PhotoModel *) = ^(LFResultObject *result, NSInteger index, PhotoModel * photoModel) {
                        if (result) {
                            [resultArray replaceObjectAtIndex:index withObject:result];
                        } else {
                            LFAsset *model = [imagePickerVc.selectedModels yx_objectAtIndex:index];
                        ...
                        [customArray addObject:photoModel];
                        dispatch_main_async_safe(^{
                            process += 1.f;
                            ...
                            if (photoEdit) {
                                [[LFPhotoEditManager manager] getPhotoWithAsset:model
                                                          ...
                                                          thumbnailCompressSize:imagePickerVc.thumbnailCompressSize
                                                                     completion:^(LFResultImage *resultImage) {
                                                                         ...
                                                                         photoModel.imageData = nil;
                                                                         photoModel.isGifImg = [Utility imageIsGifWithAsset:model.asset];
                                                                         photoModel.messageBodyType = HanTalkMessageBodyTypeImage;
                                                                         photoModel.breviaryKey =  [model.asset valueForKey:@"filename"];
                                                                         
                                                                         [[SDImageCache sharedImageCache] storeImage:photoEdit.editPosterImage forKey:photoModel.breviaryKey completion:^{
                                                                             ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                         }];
                                                                         [[SDImageCache sharedImageCache] storeImage:photoEdit.editPosterImage forKey:photoModel.originalKey completion:^{
                                                                             ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                         }];
                                                                         photoModel.originalKey = [NSString stringWithFormat:@"original-%@", photoModel.breviaryKey];
                                                                         
                                                                         if ([Utility imageIsGifWithAsset:model.asset]) {
                                                                             photoModel.originalKey = [NSString stringWithFormat:@"original-%@", photoModel.breviaryKey];
                                                                             [[SDImageCache sharedImageCache] storeImage:photoEdit.editPosterImage imageData:photoEdit.editPreviewData forKey:photoModel.originalKey toDisk:YES completion:^{
                                                                                 ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                             }];
                                                                             resultComplete(resultImage, i ,photoModel);
                                                                         }else {
                                                                             resultComplete(resultImage, i ,photoModel);
                                                                         }
                                                                     }];
                                ...
                                if (imagePickerVc.allowPickingLivePhoto && model.subType == LFAssetSubMediaTypeLivePhoto && model.closeLivePhoto == NO) {
                                    ...
                                    [[LFAssetManager manager] getPhotoWithAsset:model.asset
                                                          ...
                                                          thumbnailCompressSize:imagePickerVc.thumbnailCompressSize
                                                                     completion:^(LFResultImage *resultImage) {
                                                                         //自定义 保存图片信息保存到本地SDImageCache
                                                                         PhotoModel *photoModel = [[PhotoModel alloc]init];
                                                                         photoModel.breviaryKey = resultImage.info.name;
                                                                         photoModel.originalKey = (resultImage.subMediaType == LFImagePickerSubMediaTypeGIF) ? ([NSString stringWithFormat:@"original-%@", resultImage.info.name]): (imagePickerVc.isSelectOriginalPhoto ? [NSString stringWithFormat:@"original-%@", resultImage.info.name] : nil);
                                                                         
                                                                         
                                                                         BOOL isCache = (resultImage.subMediaType == LFImagePickerSubMediaTypeGIF)?           ([[SDImageCache sharedImageCache] diskImageDataExistsWithKey:[NSString stringWithFormat:@"original-%@", resultImage.info.name]] ): ((imagePickerVc.isSelectOriginalPhoto) ? ([[SDImageCache sharedImageCache] diskImageDataExistsWithKey:[NSString stringWithFormat:@"original-%@", resultImage.info.name]] ) : ([[SDImageCache sharedImageCache] diskImageDataExistsWithKey:photoModel.breviaryKey]));
                                                                         //判断本地是否已经存在
                                                                         if (!isCache) {
                                                                             [[SDImageCache sharedImageCache] storeImage:resultImage.thumbnailImage forKey:photoModel.breviaryKey toDisk:YES completion:^{
                                                                                 ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                             }];
                                                                             //保存到本地
                                                                             if (resultImage.subMediaType == LFImagePickerSubMediaTypeGIF) {
                                                                                 [[SDImageCache sharedImageCache] storeImage:resultImage.thumbnailImage imageData:resultImage.originalData forKey:photoModel.originalKey toDisk:YES completion:^{
                                                                                     ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                                 }];
                                                                                 photoModel.isGifImg = YES;
                                                                                 photoModel.messageBodyType = HanTalkMessageBodyTypeImage;
                                                                                 resultComplete(resultImage, i ,photoModel);
                                                                                 
                                                                             }else {
                                                                                 
                                                                                 [[SDImageCache sharedImageCache] storeImage:imagePickerVc.isSelectOriginalPhoto ? resultImage.originalImage : resultImage.thumbnailImage forKey:imagePickerVc.isSelectOriginalPhoto ? photoModel.originalKey : photoModel.breviaryKey toDisk:YES completion:^{
                                                                                     photoModel.imageData = nil;
                                                                                     photoModel.isGifImg = NO;
                                                                                     photoModel.messageBodyType = HanTalkMessageBodyTypeImage;
    //                                                                                 photoModel.breviaryImg =  resultImage.thumbnailImage;
                                                                                     resultComplete(resultImage, i ,photoModel);
                                                                                     ////[[SDImageCache sharedImageCache] setValue:nil forKey:@"memCache"];
                                                                                 }];
                                                                             }
                                                                             ...
                                                                             if (resultImage.subMediaType == LFImagePickerSubMediaTypeGIF) {
                                                                                 photoModel.isGifImg = YES;
                                                                                 photoModel.messageBodyType = HanTalkMessageBodyTypeImage;
                                                                                 photoModel.originalKey =  [NSString stringWithFormat:@"original-%@", resultImage.info.name] ;
                                                                             }else {
                                                                                 photoModel.imageData = nil;
                                                                                 photoModel.isGifImg = NO;
                                                                                 photoModel.messageBodyType = HanTalkMessageBodyTypeImage;
                            ...
                            if (videoEdit) {
                                [[LFVideoEditManager manager] getVideoWithAsset:model presetName:imagePickerVc.videoCompressPresetName completion:^(LFResultVideo *resultVideo) {
                                    if (imagePickerVc.autoSavePhotoAlbum) {
                                        /** 编辑视频保存到相册 */
                                        [[LFAssetManager manager] saveVideoToCustomPhotosAlbumWithTitle:nil videoURLs:@[resultVideo.url] complete:^(NSArray<id> *assets, NSError *error) {
                                              /** 删除原来剪辑的视频 */
                                               NSString * path = (NSString *)videoEdit.editFinalURL.relativePath;
                                            ...
                                            PhotoModel *photoModel = [[PhotoModel alloc]init];
                                            photoModel.videoUrl = resultVideo.url;
    //                                        photoModel.breviaryImg = videoEdit.editPosterImage;
                                            
                                            NSDictionary *videoDict = [Utility getVideoInfoWithSourcePath:resultVideo.url.relativePath];
                                            photoModel.seconds = [videoDict[@"duration"] integerValue];
                                            photoModel.secondsStr = [Utility getMMSSFromSS:photoModel.seconds];
                                            NSNumber *size;
                                            [videoEdit.editFinalURL getResourceValue:&size forKey:NSURLFileSizeKey error:nil];
                                            photoModel.fileSize = [size integerValue];
                                            photoModel.messageBodyType = HanTalkMessageBodyTypeShortVideo;
                                            resultComplete(resultVideo, i ,photoModel);
                                ...
                                [[LFAssetManager manager] getVideoResultWithAsset:model.asset presetName:imagePickerVc.videoCompressPresetName cache:imagePickerVc.autoVideoCache completion:^(LFResultVideo *resultVideo) {
                                    PhotoModel *photoModel = [[PhotoModel alloc]init];
                                    photoModel.breviaryKey = resultVideo.info.name;
                                    photoModel.videoUrl = resultVideo.url;
    //                                photoModel.breviaryImg = [Utility getVideoPreViewImage:photoModel.videoUrl];
                                    [[SDImageCache sharedImageCache] storeImage:[Utility getVideoPreViewImage:photoModel.videoUrl] forKey:photoModel.breviaryKey completion:nil];
                                    photoModel.secondsStr = [Utility getMMSSFromSS:resultVideo.duration];
                                    photoModel.seconds = (NSInteger)resultVideo.duration ;
                                    photoModel.fileSize = resultVideo.info.byte;
                                    photoModel.imageData = nil;
                                    photoModel.isGifImg = NO;
                                    photoModel.messageBodyType = HanTalkMessageBodyTypeShortVideo;
                                    resultComplete(resultVideo, i ,photoModel);
                    ...
                    dispatch_group_notify(_group, dispatch_get_main_queue(), ^{
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.25f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            ...
                            if (imagePickerVc.autoDismiss) {
                                ...
                                [weakSelf callDelegateMethodWithResults:resultArray andPhotoModelArr:customArray];
                            } else {
                                [weakSelf callDelegateMethodWithResults:resultArray andPhotoModelArr:customArray];
                            }
                        });
...
- (void)callDelegateMethodWithResults:(NSArray <LFResultObject *>*)results andPhotoModelArr:(NSArray <PhotoModel *> *)photoModelArray{
    ...
    } else if ([pickerDelegate respondsToSelector:@selector(lf_imagePickerController:didFinishPickingResult: didFinishPhotoModelArr: andisSelectOriginalPhoto:)]) {
        [pickerDelegate lf_imagePickerController:imagePickerVc didFinishPickingResult:results didFinishPhotoModelArr:photoModelArray andisSelectOriginalPhoto:imagePickerVc.isSelectOriginalPhoto];
                                      ...
                                      thumbnailCompressSize:imagePickerVc.thumbnailCompressSize
                                                 completion:^(LFResultImage *resultImage) {
                                                     ...
                                                     [picker.presentingViewController.presentingViewController dismissViewControllerAnimated:YES completion:^{
                                                         [self callDelegateMethodWithResults:@[resultImage] andPhotoModelArr:@[]];
                                                     }];
                                                 }];
            }else if (error) {
                [imagePickerVc hideProgressHUD];
...
#pragma clang diagnostic pop
        ...
        [self presentVc:alert animated:YES completion:nil];
    } else {
        #pragma clang diagnostic push
        #pragma clang diagnostic ignored "-Wundeclared-selector"
        if ([imagePickerVc respondsToSelector:@selector(cancelButtonClick)]) {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
            if ([imagePickerVc.pickerDelegate respondsToSelector:@selector(lf_imagePickerControllerTakePhoto:)]) {
                ...
                [self presentVc:mediaPickerController animated:YES completion:NULL];
            }
        } else {
